...
export default function Carousel({ folder = '/photos', interval = 4000 }) {
  ...
  useEffect(() => {
    ...
    async function discover() {
      ...
      for (const name of unnamedCandidates) candidates.push(`${folder}/${name}`);
  ...
  return (
    <div className="carousel-root">
      <div className="carousel-inner">
        ...
        <button className="carousel-nav prev" onClick={() => go(index - 1)} aria-label="Anterior">‹</button>
        <button className="carousel-nav next" onClick={() => go(index + 1)} aria-label="Siguiente">›</button>

        <div className="carousel-dots">
          {images.map((_, i) => (
            <button key={i} className={`dot ${i === index ? 'active' : ''}`} onClick={() => go(i)} aria-label={`Ir a ${i+1}`}></button>
          ))}
        </div>
      </div>
    </div>
